...
function appendWorkflowResult(data) {
  ...
  const dimensions = (data.dimensions || []).map((item) => `| ${item.dimension} | ${item.score}/3 | ${item.status} |`).join('\n');
  ...
  addMessage('F3', `# 评审结果：${data.overall_score}/100\n\n| 维度 | 得分 | 状态 |\n| --- | ---: | --- |\n${dimensions}\n\n## 证据缺口\n${gaps}\n\n## 修改建议\n${recommendations}\n\n## 材料边界初筛\n${claims || '暂无可拆分陈述。'}\n\n> ${data.integrity_note}\n\n**运行编号：** ${data.run_id}`, 'assistant');
}
...
function updateFlowControls() {
  ...
  $('phaseLabel').textContent = currentFlow === 'F1' ? '理论学习' : currentFlow === 'F3' ? '评审反馈' : PHASES.value_probe;
...
async function sendMessage(event) {
  ...
  if (file) addMessage(currentFlow, message, 'user', { fileName: file.name, fileSize: `${(file.size / 1024).toFixed(1)} KB` });
...
$('removeFileBtn').addEventListener('click', () => { currentFile = null; $('fileUpload').value = ''; $('filePreview').classList.add('hidden'); });
document.querySelectorAll('.flow-tab').forEach((button) => button.addEventListener('click', () => { currentFlow = button.dataset.flow; renderCurrentFlow(); updateFlowControls(); }));
