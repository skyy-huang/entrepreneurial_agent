...
async def process_cases():
    ...
    for dirpath, dirnames, filenames in os.walk(DATA_DIR):
        # 计算相对路径，例如 "2025挑战杯应用赛优秀作品/工业制造/本科"
        ...
        if len(path_parts) >= 2:
            industry = path_parts[1]
        if len(path_parts) >= 3:
            level = path_parts[2]

        for filename in filenames:
            ext = filename.split('.')[-1].lower()
            if ext not in ['pdf', 'doc', 'docx', 'txt', 'textclipping']:
                continue
            ...
            print(f"正在处理: {file_path} (行业: {industry}, 级别: {level}) ...")
